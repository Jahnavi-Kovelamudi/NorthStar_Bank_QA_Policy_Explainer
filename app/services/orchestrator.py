...
def add_agent_error(errors: list, agent_name: str, detail: str):
    errors.append({
        "agent": agent_name,
        "message": detail
    })
    ...
    try:
        ...
        try:
            retrieval_result = retrieve_documents(
                question,
                planner_result.get("domains", ["general"])
            )
        ...
        try:
            ...
            if not final_answer or not final_answer.strip():
                final_answer = (
                    "I can help with general banking guidance. For account-specific or "
                    "security-sensitive issues, please use official bank support channels."
                )
        except Exception:
            final_answer = (
                "I can help with general banking guidance. For account-specific or "
        ...
        try:
            ...
            output_result = {
                "safe": False,
                "reason": "Output guardrails failed safely."
            }
        ...
        try:
            ...
            if not isinstance(critic_feedback, dict):
                critic_feedback = get_fallback_critic_feedback(
                    "Fallback critic used due to invalid response structure."
                )
        except Exception:
            critic_feedback = get_fallback_critic_feedback(
                "Fallback critic used due to critic failure."
            )
        ...
        return {
            ...
            "final_answer": (
                ...
                "Please try again or use official bank support channels for secure assistance."
            ),
            "output_safe": False,
            "critic_feedback": get_fallback_critic_feedback(
                f"Workflow failed safely: {str(e)}"
            ),
            "trace": trace + ["workflow_fatal_error"]
        }